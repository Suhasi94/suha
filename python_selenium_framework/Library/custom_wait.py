...
class _visibility_of_element_located(visibility_of_element_located):
    def __call__(self, driver):
        ...
        # Calling __call__ method of parent class. (Re-using the parent class
        # functionality)
        ...
        if isinstance(result, WebElement):
            # Checking if the element is enabled is the extra functionality
            # that i am adding in child class
            return result.is_enabled()
        else:
            return False

# decorator for waiting before all selenium actions
def wait_(visibility=True, enabled=True, timeout=30, is_alert=False):
    def _func(func):
